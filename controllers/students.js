        ...
        .populate(
            {
                ...
                populate: {
                    ...
                    populate: {
                        ...
                        select: '_id mark checked answerFile'
                    }
                }
            }
        );
...
// @desc      fetch subjects
// @route     GET /api/v1/students/subjects/:id
// @access    Private
exports.getSubjects = asyncHandler(async (req, res, next) => {
    let subjects = await Student.findById(req.params.id)
    .select('subjects')
    .populate(
        {
            path: 'subjects',
    ...
    for (let i = 0; i < subjects.subjects.length; i++) {
        console.log('Eray ' + subjects.subjects[i]._id);
        for (let j = 0; j < subjects.subjects[i].assignments.length; j++) {

            const ans = await AssignmentAnswer.findOne({student: req.params.id, assignment: subjects.subjects[i].assignments[j]._id});

            if(ans)
            {
                console.log(ans);
                subjects.subjects[i].assignments[j].studentAnswer = ans;
                //console.log(subjects.subjects[i].assignments[j]);
            }
        }
    }
